...
- 데이터베이스 선택: NoSQL 데이터베이스인 Scylla DB 또는 MongoDB 중 선택하여 사
  용합니다.
...
1. **app 파일 생성**
   ...
   - `app.init()` 형태의 함수를 제공하여 프레임워크 빌드 시 한 줄의 코드로 초기
     화할 수 있도록 합니다.
...
3. **데이터베이스 초기화**
   - `db.go` / `db.py` 등의 파일을 생성하여 데이터베이스 초기화 로직을 구현합니
     다.
...
- PART5에선 POSTMAN이 아닌 Database에 내가 추가한 데이터가 기록되어 있는지 보여
  주시면 됩니다.
...
- 데이터베이스 설정 방법 및 실행 방법에 대한 간단한 문서를 PR 설명에 포함해야 합
  니다.
