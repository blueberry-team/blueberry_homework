...
1. **utils/jwt/refresh_token**
   - JWT 토큰 갱신 기능을 구현합니다.

2. **utils/jwt/verify_token**
   - JWT 토큰 검증 기능을 구현합니다.
...
- 다음 항목에 대한 테스트 결과를 첨부해야 합니다:
  - JWT 토큰 발급 성공 케이스
  - 토큰을 사용한 API 요청 성공 케이스
  - 토큰 없이 API 요청 시 실패 케이스
  - 만료된 토큰으로 API 요청 시 실패 케이스
  - 토큰 만료 1시간 전 `X-Token-Refresh-Required` 헤더 포함 확인
  - 토큰 갱신 전용 라우터를 통한 토큰 갱신 기능 동작 확인
