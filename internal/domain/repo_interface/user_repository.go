...
type UserRepository interface {
	// 이메일로 사용자 찾기 함수
	FindByEmail(email string) (gocql.UUID, error)

	// id로 사용자 찾기 함수
	FindById(userId gocql.UUID) (bool, error)

	// 유저의 역할을 확인하는 함수
	CheckRole(userId gocql.UUID) (string, error)
	...
	// 토큰 정보 가져오는 함수
	GetTokenInfo(userID gocql.UUID) (string, string, error)  // email, name
